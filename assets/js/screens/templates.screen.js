...
MM.templatesScreen = {
  render: function(){
    ...
    function renderRows(){
      ...
      document.querySelectorAll('.toggle-template-btn').forEach(function(btn){ btn.onclick = function(e){ var id = e.target.dataset.id; MM.state.templates = MM.state.templates.map(function(t){ if(t.id === id) t.active = !t.active; return t; }); MM.sync.syncNow().catch(function(err){ console.error('sync error:', err); MM.ui.showToast(err.message || 'Erro ao sincronizar.', 'error'); }); renderRows(); }; });
    }
