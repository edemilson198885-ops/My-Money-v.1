  ...
  render: function(){
    ...
    MM.ui.setHTML('screen-container', `
      <section class="panel section form-screen">
        ...
        <div class="muted form-note">Cadastro simples para gastos pontuais. Não replica e não usa vencimento.</div>
        ...
        <div class="field"><label>Categoria</label><select id="extra-category">${MM.helpers.renderCategoryOptions('saida', initialCategory)}</select></div>
    ...
    document.getElementById('extra-save-btn').onclick = async function(){
      try{
        ...
        var movement = MM.models.createMovement({
          ...
          category: selectedCategory || MM.helpers.inferCategory({ description: description, type: 'saida' }),
          recurrence: 'extra',
        ...
        if(purchaseDate.slice(0,7) !== MM.state.currentMonth){ throw new Error('A data da compra não pertence à competência atual. Altere a competência no topo do sistema antes de salvar.'); }
        ...
        MM.router.goTo(MM.config.SCREENS.MOVEMENTS);
      }catch(err){ MM.ui.showFeedback('extra-feedback', err.message || 'Erro ao salvar despesa extra.', 'error'); }
    };
  }
};
