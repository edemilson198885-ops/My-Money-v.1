  ...
  render: function(){
    ...
    MM.ui.setHTML('screen-container', `
      <section class="panel section form-screen">
        ...
        <div class="field"><label>Categoria</label><select id="entry-category">${MM.helpers.renderCategoryOptions('entrada', initialCategory)}</select></div>
        ...
        <div class="feedback" id="entry-feedback">Use “Automática” para o app definir a categoria sozinho pela descrição.</div>
      </section>
    `);
    ...
    document.getElementById('entry-save-btn').onclick = async function(){
      try{
        ...
        var description = document.getElementById('entry-description').value;
        var selectedCategory = document.getElementById('entry-category').value;
        var movement = MM.models.createMovement({ id: editing ? editing.id : undefined, householdId: MM.state.household.id, type: 'entrada', description: description, category: selectedCategory || MM.helpers.inferCategory({ description: description, type: 'entrada' }), recurrence: document.getElementById('entry-recurrence').value, belongsTo: (selectedUser ? selectedUser.id : ''), settledBy: editing ? editing.settledBy : '', competence: MM.state.currentMonth, amount: MM.helpers.parseCurrency(document.getElementById('entry-amount').value), dueDate: document.getElementById('entry-due-date').value, settledDate: editing ? editing.settledDate : '', note: document.getElementById('entry-note').value, origin: editing ? editing.origin : 'manual', templateId: editing ? editing.templateId : null });
        var movementMonth = movement.dueDate ? movement.dueDate.slice(0,7) : '';
        if(movementMonth !== MM.state.currentMonth){ throw new Error('A data informada não pertence à competência atual. Altere a competência no topo do sistema antes de salvar.'); }
