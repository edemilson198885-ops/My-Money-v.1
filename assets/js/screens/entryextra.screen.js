...
MM.entryExtraScreen = {
  render: function(){
    ...
    var initialCategory = editing ? (editing.category || '') : '';

    MM.ui.setHTML('screen-container', `
      <section class="panel section form-screen">
        <h2 style="margin-top:0">${editing ? 'Editar entrada extra' : 'Entrada extra'}</h2>
        <div class="muted form-note">Cadastro simples para receitas pontuais. Não replica no mês seguinte.</div>
        ...
        <div class="field"><label>Categoria</label><select id="entry-extra-category">${MM.helpers.renderCategoryOptions('entrada', initialCategory)}</select></div>
        ...
        <div class="feedback" id="entry-extra-feedback">Use “Automática” para o app definir a categoria sozinho pela descrição.</div>
      </section>
    `);
    ...
    document.getElementById('entry-extra-save-btn').onclick = async function(){
      try{
        ...
        var description = document.getElementById('entry-extra-description').value;
        var selectedCategory = document.getElementById('entry-extra-category').value;
        var movement = MM.models.createMovement({
          id: editing ? editing.id : undefined,
          householdId: MM.state.household.id,
          type: 'entrada',
          description: description,
          category: selectedCategory || MM.helpers.inferCategory({ description: description, type: 'entrada' }),
          recurrence: 'extra',
        ...
        if(receiveDate.slice(0,7) !== MM.state.currentMonth){ throw new Error('A data do recebimento não pertence à competência atual. Altere a competência no topo do sistema antes de salvar.'); }
      ...
      }catch(err){ MM.ui.showFeedback('entry-extra-feedback', err.message || 'Erro ao salvar entrada extra.', 'error'); }
    };
  }
};
