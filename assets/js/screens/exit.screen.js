  ...
  render: function(){
    ...
    MM.ui.setHTML('screen-container', `
      <section class="panel section form-screen">
        ...
        <div class="field"><label>Categoria</label><select id="exit-category">${MM.helpers.renderCategoryOptions('saida', initialCategory)}</select></div>
        ...
        <div class="feedback" id="exit-feedback">Use “Automática” para o app definir a categoria sozinho pela descrição.</div>
      </section>
    `);
    ...
    document.getElementById('exit-save-btn').onclick = async function(){
      try{
        ...
        var description = document.getElementById('exit-description').value;
        var selectedCategory = document.getElementById('exit-category').value;
        var movement = MM.models.createMovement({ id: editing ? editing.id : undefined, householdId: MM.state.household.id, type: 'saida', description: description, category: selectedCategory || MM.helpers.inferCategory({ description: description, type: 'saida' }), recurrence: document.getElementById('exit-recurrence').value, belongsTo: (selectedUser ? selectedUser.id : ''), settledBy: editing ? editing.settledBy : '', competence: MM.state.currentMonth, amount: MM.helpers.parseCurrency(document.getElementById('exit-amount').value), dueDate: document.getElementById('exit-due-date').value, settledDate: editing ? editing.settledDate : '', note: document.getElementById('exit-note').value, origin: editing ? editing.origin : 'manual', templateId: editing ? editing.templateId : null });
